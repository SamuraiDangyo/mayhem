...
void ThinkSetup(const int think_time) {
  ...
  g_naked_king = g_nullmove_on = (g_wtm && PopCount(Black()) == 1) || (!g_wtm && PopCount(White()) == 1);
  g_classical = g_naked_king || !g_nnue_exist; // vs naked king = active mate help + disable null move
}
