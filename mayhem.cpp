...
int main(int argc, char **argv) {
  mayhem::Init();
  if (argc == 1)                                             {mayhem::Loop();}
  else if (argc == 2 && std::string(argv[1]) == "--help")    {mayhem::PrintHelp();}
  else if (argc == 2 && std::string(argv[1]) == "--version") {mayhem::PrintVersion();}
  ...
  else                                                       {std::cout << "> mayhem --help" << std::endl;}
  return EXIT_SUCCESS;
}
